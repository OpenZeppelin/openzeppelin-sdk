      ...
      describe('when the network file does not have any proxies', function () {
        ...
        describe('when the app has one proxy registered', function () {
          ...
          it('reports that diff', async function () {
            ...
            this.comparator.reports[0].expected.should.be.equal('none')
            this.comparator.reports[0].observed.should.be.equal('one')
            this.comparator.reports[0].description.should.be.equal(`Missing registered proxy of Impl at ${this.proxy._address} pointing to ${this.impl._address}`)
          })
        })
        ...
        describe('when the app has many proxies registered', function () {
          ...
          it('reports that diff', async function () {
            ...
            this.comparator.reports[0].expected.should.be.equal('none')
            this.comparator.reports[0].observed.should.be.equal('one')
            this.comparator.reports[0].description.should.be.equal(`Missing registered proxy of Impl at ${this.implProxy._address} pointing to ${this.impl._address}`)
            this.comparator.reports[1].expected.should.be.equal('none')
            this.comparator.reports[1].observed.should.be.equal('one')
            this.comparator.reports[1].description.should.be.equal(`Missing registered proxy of ChildImpl at ${this.childImplProxy._address} pointing to ${this.childImpl._address}`)
          })
        })
      })
      ...
      describe('when the network file has two proxies', function () {
        ...
        describe('when the app does not have any proxy registered', function () {
          it('reports those diffs', async function () {
            ...
            this.comparator.reports[0].expected.should.be.equal('one')
            this.comparator.reports[0].observed.should.be.equal('none')
            this.comparator.reports[0].description.should.be.equal(`A proxy of Impl at 0x1 pointing to ${this.impl._address} is not registered`)
            this.comparator.reports[1].expected.should.be.equal('one')
            this.comparator.reports[1].observed.should.be.equal('none')
            this.comparator.reports[1].description.should.be.equal(`A proxy of Impl at 0x2 pointing to ${this.impl._address} is not registered`)
          })
        })
        ...
        describe('when the app has one proxy registered', function () {
          ...
          describe('when it matches one proxy address', function () {
            ...
            describe('when it matches the alias but not the implementation address', function () {
              ...
              it('reports that diff', async function () {
                ...
                this.comparator.reports.should.have.lengthOf(2)
                this.comparator.reports[0].expected.should.be.equal(this.childImpl._address)
                this.comparator.reports[0].observed.should.be.equal(this.impl._address)
                this.comparator.reports[0].description.should.be.equal(`Pointed implementation of Impl proxy at ${this.proxy._address} does not match`)
                this.comparator.reports[1].expected.should.be.equal('one')
                this.comparator.reports[1].observed.should.be.equal('none')
                this.comparator.reports[1].description.should.be.equal(`A proxy of Impl at 0x1 pointing to ${this.impl._address} is not registered`)
              })
            })
            ...
            describe('when it matches the implementation address but not the alias', function () {
              beforeEach('changing network file', async function () {
                ...
                this.networkFile.setProxies(packageName, 'Impl', [{ implementation: this.impl._address, address: '0x1', version: '1.0.0' }])
                this.networkFile.setProxies(packageName, 'ChildImpl', [{ implementation: this.impl._address, address: this.proxy._address, version: '1.0.0' }])
              })
              ...
              it('reports that diff', async function () {
                ...
                this.comparator.reports[0].description.should.be.equal(`Alias of proxy at ${this.proxy._address} pointing to ${this.impl._address} does not match`)
                this.comparator.reports[1].expected.should.be.equal('one')
                this.comparator.reports[1].observed.should.be.equal('none')
                this.comparator.reports[1].description.should.be.equal(`A proxy of Impl at 0x1 pointing to ${this.impl._address} is not registered`)
              })
            ...
            describe('when it does not match the alias and the implementation address', function () {
              beforeEach('changing network file', async function () {
                const { name: packageName } = this.packageFile
                this.networkFile.setProxies(packageName, 'Impl', [{ implementation: this.impl._address, address: '0x1', version: '1.0.0' }])
                this.networkFile.setProxies(packageName, 'ChildImpl', [{ implementation: this.childImpl._address, address: this.proxy._address, version: '1.0.0' }])
              })
              ...
              it('reports that diff', async function () {
                ...
                this.comparator.reports[0].observed.should.be.equal('Impl')
                this.comparator.reports[0].description.should.be.equal(`Alias of proxy at ${this.proxy._address} pointing to ${this.impl._address} does not match`)
                this.comparator.reports[1].expected.should.be.equal(this.childImpl._address)
                this.comparator.reports[1].observed.should.be.equal(this.impl._address)
                this.comparator.reports[1].description.should.be.equal(`Pointed implementation of Impl proxy at ${this.proxy._address} does not match`)
                this.comparator.reports[2].expected.should.be.equal('one')
                this.comparator.reports[2].observed.should.be.equal('none')
                this.comparator.reports[2].description.should.be.equal(`A proxy of Impl at 0x1 pointing to ${this.impl._address} is not registered`)
              })
            })
          })
          ...
          describe('when it does not match any proxy address', function () {
            it('reports those diffs', async function () {
              ...
              this.comparator.reports[2].expected.should.be.equal('one')
              this.comparator.reports[2].observed.should.be.equal('none')
              this.comparator.reports[2].description.should.be.equal(`A proxy of Impl at 0x2 pointing to ${this.impl._address} is not registered`)
            })
          })
        })
      })
