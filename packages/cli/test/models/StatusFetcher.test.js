...
contract('StatusFetcher', async function([_, owner, anotherAddress]) {
  ...
  describe('app', function() {
    beforeEach(
      'initializing network file and status checker',
      async function() {
    ...
    describe('package', function() {
      describe('when the network file shows a different package address than the one set in the App contract', function() {
        ...
        it('updates the package address', async function() {
          ...
          this.networkFile.packageAddress.should.have.be.equal(
            this.project.package.address,
          );
        });
      });
    ...
    describe('provider', function() {
      describe('when the network file shows a different provider address than the one set in the App contract', function() {
        ...
        it('updates the provider address', async function() {
          ...
          this.networkFile.providerAddress.should.have.be.equal(
    ...
    describe('dependencies', function() {
      ...
      describe('when the app project does not have dependencies', function() {
        describe('when the network file has dependencies', function() {
          it('removes the local dependency', async function() {
            this.networkFile.setDependency('an-awesome-dependency', {
              version: '1.0.0',
              package: '0x01',
            });
            await this.checker.checkDependencies();

            this.networkFile.hasDependency(
              'an-awesome-dependency',
            ).should.be.false;
          });
        });
      });
      ...
      describe('when the app project has dependencies', function() {
        beforeEach('set project with multiple dependencies', async function() {
          const dependencies = [
            `${this.dep1.name}@${this.dep1.version}`,
            `${this.dep2.name}@${this.dep2.version}`,
          ];
          ...
          this.dep1 = {
            ...this.dep1,
            address: this.networkFile.getDependency(this.dep1.name).package,
          };
          this.dep2 = {
            ...this.dep2,
            address: this.networkFile.getDependency(this.dep2.name).package,
          };
        });
        ...
        describe('when the network file does not have matching dependencies', function() {
          it('adds dependencies to network file', async function() {
            ...
            this.networkFile
              .getDependency(this.dep1.name)
              .version.should.eq(this.dep1.version);
            this.networkFile
              .getDependency(this.dep2.name)
              .version.should.eq(this.dep2.version);
            this.networkFile
              .getDependency(this.dep1.name)
              .package.should.eq(this.dep1.address);
            this.networkFile
              .getDependency(this.dep2.name)
              .package.should.eq(this.dep2.address);
          });
        });
        ...
        describe('when the network file has a dependency with wrong dependency address', function() {
          it('fixes the dependency address', async function() {
            ...
            this.networkFile.updateDependency(this.dep1.name, dependency => ({
              ...dependency,
              package: '0x01',
            }));
            ...
            this.networkFile
              .getDependency(this.dep1.name)
              .package.should.eq('0x01');
            await this.checker.checkDependencies();
            this.networkFile
              .getDependency(this.dep1.name)
              .package.should.eq(address);
          });
        });
        ...
        describe('when the network file has other dependency that is not deployed', function() {
          it('removes that dependency', async function() {
            ...
            this.networkFile.setDependency(dependencyName, {
              package: '0x01',
              version: '1.0.0',
            });
    ...
    describe('implementations', function() {
      describe('when the network file does not have any contract', function() {
        ...
        describe('when the directory of the current version has one contract', function() {
          describe('when the contract alias and contract name are the same', function() {
            beforeEach(
              'registering new implementation in AppDirectory',
              async function() {
                this.impl = await this.project.setImplementation(
                  ImplV1,
                  'ImplV1',
                );
              },
            ...
            it('adds that contract', async function() {
              ...
              this.networkFile
                .contract('ImplV1')
                .address.should.be.equal(this.impl.address);
              this.networkFile
                .contract('ImplV1')
                .deployedBytecodeHash.should.be.equal(
                  bytecodeDigest(ImplV1.schema.bytecode),
                );
              this.networkFile
                .contract('ImplV1')
                .constructorCode.should.be.equal(constructorCode(ImplV1));
              this.networkFile
                .contract('ImplV1')
                .bodyBytecodeHash.should.be.equal(
                  bytecodeDigest(bodyCode(ImplV1)),
                );
            });
          });
          ...
          describe('when the contract alias and contract name are different', function() {
            ...
            it('adds that contract', async function() {
              ...
              this.networkFile
                .contract('Impl')
                .deployedBytecodeHash.should.be.equal('unknown');
              this.networkFile
                .contract('Impl')
                .constructorCode.should.be.equal('unknown');
              this.networkFile
                .contract('Impl')
                .bodyBytecodeHash.should.be.equal(
                  bytecodeDigest(bodyCode(ImplV1)),
        ...
        describe('when the directory of the current version has many contracts', function() {
          beforeEach(
            'registering two new implementations in AppDirectory',
            async function() {
              this.impl = await this.project.setImplementation(
                ImplV1,
                'ImplV1',
              );
              this.childImpl = await this.project.setImplementation(
                ChildImplV1,
                'ChildImplV1',
              );
            },
          ...
          it('adds those contracts', async function() {
            ...
            this.networkFile
              .contract('ChildImplV1')
              .address.should.be.equal(this.childImpl.address);
            this.networkFile
              .contract('ChildImplV1')
              .deployedBytecodeHash.should.be.equal(
                bytecodeDigest(ChildImplV1.schema.bytecode),
              );
            this.networkFile
              .contract('ChildImplV1')
              .constructorCode.should.be.equal(constructorCode(ChildImplV1));
            this.networkFile
              .contract('ChildImplV1')
              .bodyBytecodeHash.should.be.equal(
                bytecodeDigest(bodyCode(ChildImplV1)),
              );
          });
        });
      ...
      describe('when the network file has some contracts', function() {
        beforeEach('adding some contracts', async function() {
          ...
          this.networkFile.addContract(
            'Impl',
            this.impl,
            getStorageLayout(ImplV1),
          );
          this.networkFile.addContract(
            'ChildImpl',
            this.childImpl,
            getStorageLayout(ChildImplV1),
          );
        });
        ...
        describe('when the directory of the current version has one of those contract', function() {
          describe('when the directory has the same address and same bytecode for that contract', function() {
            ...
            it('removes the unregistered contract without changing the registered one', async function() {
              ...
              this.networkFile
                .contract('Impl')
                .address.should.be.equal(previousContract.address);
              this.networkFile
                .contract('Impl')
                .deployedBytecodeHash.should.be.equal(
                  previousContract.deployedBytecodeHash,
                );
              this.networkFile
                .contract('Impl')
                .constructorCode.should.be.equal(
                  previousContract.constructorCode,
                );
              this.networkFile
                .contract('Impl')
                .bodyBytecodeHash.should.be.equal(
                  previousContract.bodyBytecodeHash,
                );
            });
          });
          ...
          describe('when the bytecode for that contract is different', function() {
            beforeEach(
              'registering new implementation in AppDirectory',
              async function() {
                this.networkFile.updateImplementation(
                  'Impl',
                  implementation => ({
                    ...implementation,
                    bodyBytecodeHash: '0x0',
                  }),
                );
                await this.directory.setImplementation(
                  'Impl',
        ...
        describe('when the directory of the current version has both contracts', function() {
          ...
          it('does not update the contracts list', async function() {
            ...
            const previousChildImplContract = this.networkFile.contract(
              'ChildImpl',
            );
            ...
            this.networkFile
              .contract('Impl')
              .address.should.be.equal(previousImplContract.address);
            this.networkFile
              .contract('Impl')
              .deployedBytecodeHash.should.be.equal(
                previousImplContract.deployedBytecodeHash,
              );
            this.networkFile
              .contract('Impl')
              .constructorCode.should.be.equal(
                previousImplContract.constructorCode,
              );
            this.networkFile
              .contract('ChildImpl')
              .address.should.be.equal(previousChildImplContract.address);
            this.networkFile
              .contract('ChildImpl')
              .deployedBytecodeHash.should.be.equal(
                previousChildImplContract.deployedBytecodeHash,
              );
            this.networkFile
              .contract('ChildImpl')
              .constructorCode.should.be.equal(
                previousChildImplContract.constructorCode,
              );
          });
        });
