...
import {
  checkVersion,
  isMigratableManifestVersion,
} from '../../src/models/files/ManifestVersion';
    ...
    describe('checkVersion', function() {
      ...
      context('when version is undefined', function() {
        it('throws error', function() {
          (() => checkVersion(undefined, projectFileName)).should.throw(
            /Manifest version identifier not found in/,
          );
        });
      });
      ...
      context('when version does not exact major and minor', function() {
        it('throws error on different major', function() {
          (() => checkVersion('3.2', projectFileName)).should.throw(
            /Unrecognized manifest version identifier/,
          );
        });

        it('throws error on different minor', function() {
          (() => checkVersion('2.3', projectFileName)).should.throw(
            /Unrecognized manifest version identifier/,
          );
        });
      });
      ...
      context(
        ...
        function() {
          it('does not fail nor returns true', function() {
            (() => checkVersion('2', projectFileName)).should.not.throw(
              /Unrecognized manifest version identifier/,
            );
            (() => checkVersion('2', projectFileName)).should.not.throw(
              /manifest version identifier not found in/,
