...
contract('Stdlib', function ([_, owner]) {
  ...
  const deployedAddress = '0x10';

  beforeEach("initializing stdlib", async function () {
    this.stdlib = new Stdlib(packageName, owner);
  });
  ...
  describe('getContract', function () {
    ...
    it('should retrieve a greeter contract', async function () {
      should.exist(this.greeter);
      this.greeter.bytecode.should.eq(greeterBytecode);
    });
  
    it('should set web3 provider', async function () {
      this.greeter.currentProvider.host.should.be.not.null;
    });

    it('should set gas defaults', async function () {
  ...
  describe('getDeployed', function () {
    it('should return deployment address for specified network', async function () {
      this.stdlib.getDeployed('testnet').should.eq(deployedAddress)
    });
  });
