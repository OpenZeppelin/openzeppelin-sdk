  ...
  describe('validations', function() {
    context('when not specifying proxy address', function() {
      it('throws an error', async function() {
        await call({ network, txParams, networkFile: this.networkFile, methodName: 'initialize', methodArgs: [42] })
    ...
    context('when specifying a non-existent proxy address', function() {
      it('throws an error', async function() {
        ...
        await call({ network, txParams, networkFile: this.networkFile, proxyAddress, methodName: 'initialize', methodArgs: [42] })
    ...
    context('when not specifying a method name', function() {
      it('throws an error', async function() {
        const proxyAddress = '0x124';
        await call({ network, txParams, networkFile: this.networkFile, proxyAddress, methodArgs: [42] })
  ...
  describe('errors', function() {
    context('when specifying a wrong number of method arguments', function() {
      it('throws an error', async function() {
        ...
        await call({ network, txParams, networkFile: this.networkFile, proxyAddress, methodName: 'initialize', methodArgs: [42, 44] })
  ...
  describe('specifying valid address, method name and method args', function() {
    context('when calling a variable getter', function() {
      it('calls the getter and logs the returned value', async function() {
        ...
        await call({ network, txParams, networkFile: this.networkFile, proxyAddress, methodName: 'value', methodArgs: [] });

        this.logs.infos[this.logs.infos.length - 1].should.eq(`Call returned: 42`);
      });
    });
    ...
    context('when calling a regular method', function() {
      context('when the method does not return', function() {
        it('calls the method and logs that it has been successfully called', async function() {
          const proxyAddress = this.networkFile.getProxies({ contract: 'Impl'})[0].address;
          await call({ network, txParams, networkFile: this.networkFile, proxyAddress, methodName: 'doesNotReturn', methodArgs: [] });

          this.logs.infos[this.logs.infos.length - 1].should.eq('Method doesNotReturn successfully called.');
        });
      });
      ...
      context('when the method returns a value', function() {
        it('calls the method and logs the returned value', async function() {
          const proxyAddress = this.networkFile.getProxies({ contract: 'Impl'})[0].address;
          await call({ network, txParams, networkFile: this.networkFile, proxyAddress, methodName: 'say', methodArgs: [] });
