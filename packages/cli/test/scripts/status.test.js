...
contract('status script', function([_, owner]) {
  ...
  const shouldDescribeApp = function() {
    describe('root app', function() {
      ...
      it('should log plain app info', async function() {
        ...
        await this.capturingLogs(
          status({ network, networkFile: this.networkFile }),
        );
  ...
  const shouldDescribeContracts = function() {
    describe('contracts', function() {
      ...
      it('should not log contract name when matches alias', async function() {
        await push({ network, txParams, networkFile: this.networkFile });
        await add({
          contractsData: [{ name: anotherContractName }],
      ...
      it('should log out-of-sync contract', async function() {
        ...
        await push({ network, txParams, networkFile: this.networkFile });
        await add({
          contractsData: [{ name: anotherContractName, alias: contractAlias }],
          packageFile: this.packageFile,
        });
        await this.capturingLogs(
          status({ network, networkFile: this.networkFile }),
  ...
  const shouldDescribeUnlinkedDependency = function() {
    it('should log missing library', async function() {
      ...
      this.logs.text.should.match(
        /mock-stdlib@1.1.0 is required but is not linked/i,
      );
    });
  };
  ...
  const shouldDescribeDependency = function() {
    describe('dependency', function() {
      it('should log connected dependency', async function() {
        ...
        this.networkFile.updateDependency('mock-stdlib-undeployed', dep => ({
          ...dep,
          customDeploy: false,
        }));
        await this.capturingLogs(
      ...
      it('should log connected dependency when semver requirement matches', async function() {
        await link({
          packageFile: this.packageFile,
          dependencies: ['mock-stdlib-undeployed@^1.0.0'],
          installLib: false,
        });
        await push({
          network,
        ...
        this.logs.text.should.match(
          /mock-stdlib-undeployed@\^1\.0\.0 is linked to version 1\.1\.0/i,
        );
      });
      ...
      it('should log connected dependency to incorrect version', async function() {
        ...
        this.logs.text.should.match(
          /mock-stdlib-undeployed@\^2\.0\.0 is linked to a different version \(1\.1\.0\)/i,
        );
      });
      ...
      it('should log deployed dependency', async function() {
        ...
        this.logs.text.should.match(
          /mock-stdlib-undeployed@1\.1\.0 is linked to a custom deployment/i,
        );
      });
      ...
      it('should log deployed dependency to incorrect version', async function() {
        ...
        this.logs.text.should.match(
          /mock-stdlib-undeployed@\^2\.0\.0 is linked to a custom deployment of a different version \(1\.1\.0\)/i,
        );
      });
    });
  };
  ...
  const shouldNotModifyPackage = function() {
    it('should not deploy a new package', async function() {
      await push({ network, txParams, networkFile: this.networkFile });
      const packageAddress = new LocalController(this.packageFile).onNetwork(
        network,
        txParams,
        this.networkFile,
      ).packageAddress;
      ...
      new LocalController(this.packageFile)
        .onNetwork(network, txParams, this.networkFile)
        .packageAddress.should.eq(packageAddress);
    });
  };
