...
contract('set-admin script', function(accounts) {
  ...
  describe('on application contract', function () {
    ...
    it('changes admin of a proxy given its address', async function() {
      ...
      await assertAdmin(this.impl2._address, this.networkFile.proxyAdminAddress, this.networkFile)
      await assertAdmin(this.withLibraryImpl._address, this.networkFile.proxyAdminAddress, this.networkFile)
    });
  ...
  describe('on dependency contract', function () {
    ...
    it('changes admin of a proxy given its address', async function() {
      ...
      await assertAdmin(this.greeter2._address, this.networkFile.proxyAdminAddress, this.networkFile)
    });
    ...
    it('changes no admins if package is missing', async function() {
      ...
      await assertAdmin(this.greeter1._address, this.networkFile.proxyAdminAddress, this.networkFile)
      await assertAdmin(this.greeter2._address, this.networkFile.proxyAdminAddress, this.networkFile)
    });
  });
});
