...
contract('query-signed-deployment script', function([_, owner, another, admin]) {
  ...
  const shouldHandleQuerySignedDeploymentScript = function() {
    ...
    it('should query deployment address from signature', async function() {
      ...
      const predictedAddress = await queryDeployment({ network, txParams, networkFile, salt, sender: helpers.signer });
      ...
      const address = await querySignedDeployment({ signature, salt, contractAlias, admin, network, txParams, networkFile });
