...
contract('Truffle', () => {
  ...
  describe('validateAndLoadNetworkConfig', function() {
    context(
      'when the requested network does not exist in the truffle config file',
      function() {
        context('when truffle config file is truffle.js', function() {
          it('throws an error', function() {
            ...
            (() =>
              Truffle.validateAndLoadNetworkConfig(
                'non-existent',
                true,
                testDir,
              )).should.throw(/is not defined in your truffle.js file/);
          });
        });
          ...
          it('throws an error', function() {
            ...
            (() =>
              Truffle.validateAndLoadNetworkConfig(
                ...
                true,
                testDir,
              )).should.throw(/is not defined in your truffle-config.js file/);
          });
        });
      },
    );
  ...
  describe('getProviderAndDefaults', function() {
    ...
    context('when using truffle-hdwallet-provider', function() {
      beforeEach(function() {
        ...
        sinon
          .stub(Truffle, 'getConfig')
          .returns({ provider: { constructor: { name: 'HDWalletProvider' } } });
    ...
    context('with directory created', function() {
      ...
      context('with contracts', function() {
        it('returns an array with items inside', function() {
          FileSystem.writeJson(`${testDir}/build/contracts/Foo.json`, {
            sourcePath: `${testDir}/contracts`,
            bytecode: '0x124',
          });
          FileSystem.writeJson(`${testDir}/build/contracts/Bar.json`, {
            sourcePath: `${testDir}/contracts`,
            bytecode: '0x',
          });
          FileSystem.writeJson(`${testDir}/build/contracts/Buz.json`, {
            sourcePath: `other-directory/contracts`,
            bytecode: '0x124',
          });
          const contractNames = Truffle.getContractNames();
