...
export default class StatusComparator {
  ...
  public onUnregisteredLocalProxy(expected: string, observed: string, { alias, address, implementation }: ComparedObject): void {
    this._addReport(expected, observed, `A proxy of ${alias} at ${address} pointing to ${implementation} is not registered`);
  }

  public onMissingRemoteProxy(expected: string, observed: string, { alias, address, implementation }: ComparedObject): void {
    this._addReport(expected, observed, `Missing registered proxy of ${alias} at ${address} pointing to ${implementation}`);
  }

  public onMismatchingProxyAlias(expected: string, observed: string, { address, implementation }: ComparedObject): void {
    this._addReport(expected, observed, `Alias of proxy at ${address} pointing to ${implementation} does not match`);

  }
  public onMismatchingProxyImplementation(expected: string, observed: string, { alias, address }: ComparedObject): void {
  ...
  public onMissingDependency(expected: string, observed: string, { name, address }: ComparedObject): void {
