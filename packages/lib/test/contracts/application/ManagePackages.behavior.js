...
export default function shouldManagePackages(accounts) {
  ...
  const assertPackage = async function(packageName, expectedAddress, expectedVersion) {
    const [address, version] = await this.app.methods.getPackage(packageName).call();
  ...
  describe('setPackage', function () {  
    beforeEach(async function () {
      ...
      await this.package.methods.addVersion(version_1, this.directoryV1.address, contentURI).send({ from: packageOwner });
      this.anotherPackage = await Package.new({ from: packageOwner })
      await this.anotherPackage.methods.addVersion(version_0, this.directory.address, contentURI).send({ from: packageOwner });
    ...
    it('registers a new package and version', async function () {
      await this.app.methods.setPackage(this.anotherPackageName, this.anotherPackage.address, version_0).send({ from: appOwner });
    ...
    it('overwrites registered package with the same name', async function () {
      await this.app.methods.setPackage(this.packageName, this.anotherPackage.address, version_0).send({ from: appOwner });
    ...
    it('updates existing package version', async function () {
      await this.app.methods.setPackage(this.packageName, this.package.address, version_1).send({ from: appOwner });
    ...
    it('fails if package does not have the required version', async function () {
      await assertRevert(this.app.methods.setPackage(this.anotherPackageName, this.anotherPackage.address, [9,9,9]).send({ from: appOwner }));
    });

    it('fails if called from non-owner account', async function () {
      await assertRevert(this.app.methods.setPackage(this.anotherPackageName, this.anotherPackage.address, version_0).send({ from: anotherAccount }));
    });
  })
  ...
  describe('unsetPackage', function () {
    ...
    it('unsets a package', async function () {
      ...
      const [address, version] = await this.app.methods.getPackage(this.packageName).call();
  ...
  describe('getProvider', function () {
    beforeEach(async function () {
      ...
      this.anotherPackage = await Package.new({ from: packageOwner })
      await this.anotherPackage.methods.addVersion(version_1, this.anotherDirectory.address, contentURI).send({ from: packageOwner });
      this.anotherPackageName = 'AnotherPackage';
      await this.app.methods.setPackage(this.anotherPackageName, this.anotherPackage.address, version_1).send({ from: appOwner });
    });
    ...
    it('returns provider from updated package', async function () {
      await this.app.methods.setPackage(this.packageName, this.anotherPackage.address, version_1).send({ from: appOwner });
      const provider = await this.app.methods.getProvider(this.packageName).call();
      provider.should.eq(this.anotherDirectory.address);
    });

    it('returns provider from updated package version', async function () {
      await this.package.methods.addVersion(version_1, this.anotherDirectory.address, contentURI).send({ from: packageOwner });
      await this.app.methods.setPackage(this.packageName, this.package.address, version_1).send({ from: appOwner });
      const provider = await this.app.methods.getProvider(this.packageName).call();
      provider.should.eq(this.anotherDirectory.address);
    });
