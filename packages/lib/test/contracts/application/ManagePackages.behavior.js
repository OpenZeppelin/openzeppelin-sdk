...
export default function shouldManagePackages(accounts) {
  ...
  describe('setPackage', function () {  
    ...
    it('registers a new package and version', async function () {
      await this.app.methods.setPackage(this.anotherPackageName, this.anotherPackage._address, version_0).send({ from: appOwner });
      await this.assertPackage(this.anotherPackageName, this.anotherPackage._address, version_0);
      await this.assertPackage(this.packageName, this.package._address, version_0);
    });

    it('overwrites registered package with the same name', async function () {
      await this.app.methods.setPackage(this.packageName, this.anotherPackage._address, version_0).send({ from: appOwner });
      await this.assertPackage(this.packageName, this.anotherPackage._address, version_0);
    });

    it('updates existing package version', async function () {
      await this.app.methods.setPackage(this.packageName, this.package._address, version_1).send({ from: appOwner });
      await this.assertPackage(this.packageName, this.package._address, version_1);
    });

    it('fails if package does not have the required version', async function () {
      await assertRevert(this.app.methods.setPackage(this.anotherPackageName, this.anotherPackage._address, [9,9,9]).send({ from: appOwner }));
    });

    it('fails if called from non-owner account', async function () {
      await assertRevert(this.app.methods.setPackage(this.anotherPackageName, this.anotherPackage._address, version_0).send({ from: anotherAccount }));
    });
  })
  ...
  describe('getProvider', function () {
    ...
    it('returns provider from package', async function () {
      ...
      provider.should.eq(this.directory._address);
    });
    ...
    it('returns provider from another package', async function () {
      ...
      provider.should.eq(this.anotherDirectory._address);
    });

    it('returns provider from updated package', async function () {
      await this.app.methods.setPackage(this.packageName, this.anotherPackage._address, version_1).send({ from: appOwner });
      const provider = await this.app.methods.getProvider(this.packageName).call();
      provider.should.eq(this.anotherDirectory._address);
    });

    it('returns provider from updated package version', async function () {
      await this.package.methods.addVersion(version_1, this.anotherDirectory._address, contentURI).send({ from: packageOwner });
      await this.app.methods.setPackage(this.packageName, this.package._address, version_1).send({ from: appOwner });
      const provider = await this.app.methods.getProvider(this.packageName).call();
      provider.should.eq(this.anotherDirectory._address);
    });
