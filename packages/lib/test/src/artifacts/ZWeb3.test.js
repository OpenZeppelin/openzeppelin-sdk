...
contract('ZWeb3', function(accounts) {
  ...
  const shouldBehaveLikeWeb3Instance = (account, receiverAccount) => {
    ...
    it('calculates sha3', function() {
      ZWeb3.sha3('something').should.be.eq(
    ...
    describe('get code', function() {
      it('can tell the deployed bytecode of a certain address', async function() {
        ...
        bytecode.should.be.equal(
          this.DummyImplementation.schema.linkedDeployedBytecode,
        );
      });
    });
    ...
    describe('estimate gas', function() {
      it('can estimate the gas of a call', async function() {
        const {
          gasUsed: expectedGas,
        } = this.impl.deployment.transactionReceipt;
        const gas = await ZWeb3.estimateGas({
          data: this.DummyImplementation.schema.linkedBytecode,
        });
    ...
    describe('transactions', function() {
      beforeEach('sending transaction', async function() {
        ...
        const receipt = await ZWeb3.sendTransaction({
          from: accounts[0],
          to: receiverAccount,
          value,
        });
      ...
      describe('send transaction', function() {
        it('can send a transaction', async function() {
          (await ZWeb3.getBalance(receiverAccount)).should.eq(
            (101e18).toString(),
          );
        });
      });
      ...
      describe('get transaction receipt with timeout', function() {
        ...
        describe('when the transaction receipt fails continuously', function() {
          beforeEach('stub ZWeb3', function() {
            sinon
              .stub(ZWeb3, 'getTransactionReceipt')
              .throws('Error', 'unknown transaction');
          });

          it('fails', async function() {
            try {
              await ZWeb3.getTransactionReceiptWithTimeout(
                this.txHash,
                this.timeout,
              );
