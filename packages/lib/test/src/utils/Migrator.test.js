  ...
  describe('migrate proxy function', function() {
    context('with valid owner', function() {
      it('changes the proxy admin', async function() {
        await migrate(this.app.address, this.proxyAddress, proxyAdminAddress, { from: owner });
    ...
    context('with invalid owner', function() {
      it('reverts migration', async function() {
        await assertRevert(migrate(this.app.address, this.proxyAddress, proxyAdminAddress, { from: anotherAccount }));
      });
    });

    context('with invalid proxy address', function() {
      it('reverts migration', async function() {
        await assertRevert(migrate(this.app.address, anotherAddress, proxyAdminAddress, { from: owner }));
      });
    });
  });
});
