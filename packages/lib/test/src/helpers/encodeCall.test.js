...
import encodeCall, { decodeCall, SCIENTIFIC_NOTATION_PATTERN } from '../../../src/helpers/encodeCall'
...
describe.only('encodeCall', () => {
  ...
  describe('when the specified type is a number (int, uint, etc)', () => {
    it('should accept valid values', () => {
      ...
      assertGoodEncoding(['int'], ['-9e17'])
      assertGoodEncoding(['uint'], ['9e18'])
    ...
    it('should throw on non integer numbers', () => {
      assertBadEncoding(['uint'], [.14], /invalid number value/)
      assertBadEncoding(['int'], [-3.14], /invalid number value/)
  ...
  describe('when the specified type is an address', () => {
    ...
    it('should throw when an invalid address is passed', () => {
      ...
      assertBadEncoding(['address'], ['0x00000000000000000000000000000000000000000000000000000000f8a8fd6d'], /invalid address/)
    })
  ...
  describe('when the specified type is an array', () => {
    it('should accept valid values', () => {
      ...
      assertGoodEncoding(['address[]'], [['0x39af68cf04abb0ef8f9d8191e1bd9c041e80e18e', '0x39af68cf04abb0ef8f9d8191e1bd9c041e80e18e']])
      assertGoodEncoding(['bool[]'], [[true, false]])

      // FIX: ethereumjs-abi does not support array of strings yet
      // assertGoodEncoding(['string[]'], [['hello', '']])
    })
    ...
    // FIX: ethereumjs-abi does not handle this scenario yet
    xit('should throw when the number of elements is lower than the array size', () => {
      assertBadEncoding(['uint[2]'], [[1]], /missing argument/)
    })

    it('should throw when the number of elements exceeds the array size', () => {
      assertBadEncoding(['uint[2]'], [[1, 2, 3]], /exceed array size/)
    })
  })

  // FIX: ethereumjs-abi does not support tuples yet
  xdescribe('when the specified type is a tuple', function() {
...
function assertGoodEncoding(types, values) {
  ...
  const decoded = decodeCall(types, encoded)
  ...
  _.zipWith(values, decoded, (value, decodedValue) => {
    if (typeof(value) === 'string' && value.match(SCIENTIFIC_NOTATION_PATTERN)) value = new BN(value)
