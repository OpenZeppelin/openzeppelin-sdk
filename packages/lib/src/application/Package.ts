...
export default class Package {
  ...
  public async hasVersion(version: string): Promise<boolean> {
    return this.packageContract.methods
      .hasVersion(toSemanticVersion(version))
      .call();
  }
  ...
  public async getDirectory(
    version: string,
  ): Promise<ImplementationDirectory | never> {
    if (!version)
      throw Error(
        'Cannot get a directory from a package without specifying a version',
      );
    const directoryAddress = await this.packageContract.methods
      .getContract(toSemanticVersion(version))
      .call();
