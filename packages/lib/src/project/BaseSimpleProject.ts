...
export default abstract class BaseSimpleProject {
  ...
  protected async _setUpgradeParams(proxyAddress: string, contractClass: ZosContract, { packageName, contractName, initMethod: initMethodName, initArgs, redeployIfChanged }: ContractInterface = {}): Promise<any> {
    const implementationAddress = await this._getOrDeployImplementation(contractClass, packageName, contractName, redeployIfChanged);
  ...
  protected _getAndLogInitCallData(contractClass: ZosContract, initMethodName?: string, initArgs?: string[], implementationAddress?: string, actionLabel?: string): string | null {
    if (initMethodName) {
