    ...
    describe('when the proposed owner is not the zero address', function() {
      ...
      describe('when the sender is the owner', function() {
        ...
        it('transfers the ownership', async function() {
          ...
          const contractOwner: string = await this.ownable.methods
            .owner()
            .call();
        ...
        it('emits an event', async function() {
          const { events } = await this.ownable.methods
      ...
      describe('when the sender is not the owner', function() {
        ...
        it('reverts', async function() {
          await assertRevert(
            this.ownable.methods.transferOwnership(newOwner).send({ from }),
          );
        });
      });
    });
