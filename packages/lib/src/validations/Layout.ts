...
export type StorageEntryComparison =
  | 'equal'
  | 'rename'
  | 'typechange'
  | 'replace';
...
export function compareStorageLayouts(
  original: StorageLayoutInfo,
  updated: StorageLayoutInfo,
): Operation[] {
  const areMatch: ComparisonFn = (var1: StorageInfo, var2: StorageInfo) =>
    storageEntryMatches(var1, var2, original.types, updated.types);
  const areEqual: EqualFn = (var1: StorageInfo, var2: StorageInfo) =>
    areMatch(var1, var2) === 'equal';

  const distanceMatrix: number[][] = levenshtein(
    original.storage,
    updated.storage,
    areEqual,
  );
  const operations: Operation[] = walk(
    distanceMatrix,
    original.storage,
    updated.storage,
    areMatch,
...
function levenshtein(
  ...
  const insertionCost: CostFunction = (_i, j) =>
    j > a.length ? 0 : INSERTION_COST;
  const diagonalCost: CostFunction = (i, j) =>
    areEqualFn(a[i - 1], b[j - 1]) ? 0 : SUBSTITUTION_COST;
...
function walk(
  ...
  while (i > 0 || j > 0) {
    ...
    if (
      ...
      throw Error(
        `Could not walk matrix at position ${i},${j}:\n${(matrix as any)
          .map(util.inspect)
          .join('\n')}\n`,
      );
  }
