...
At the end of the xref:deploying.adoc[previous guide] we deployed an OpenZeppelin SDK project with one contract. Here is the code of the contract, to keep it fresh on our minds:
...
Remember that for this guide we are using a https://truffleframework.com/docs/ganache/quickstart[ganache] local development network. Do not stop the `ganache-cli` command that xref:deploying.adoc#deploying-your-project[we ran before], or you will lose your previous deployment!
