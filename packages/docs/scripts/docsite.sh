...
if [ "$1" = "build" ]; then
  ...
  npx concurrently \
    'nodemon --delay 1 -e "*" -w ../packages/lib/contracts -w "*.hbs" -x npm run prepare-docs' \
    'env DISABLE_PREPARE_DOCS= nodemon --delay 1 -e adoc,yml -i local-playbook.yml openzeppelin-docs/build-local.js' \
