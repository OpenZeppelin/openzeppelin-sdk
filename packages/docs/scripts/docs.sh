...
if [ "$1" = "build" ]; then
  ...
  npx concurrently \
    'nodemon --delay 1 -e "*" -w ../packages/lib/contracts -w templates -x npm run prepare-docs' \
    'env DISABLE_PREPARE_DOCS= nodemon --delay 1 -e adoc,yml -i openzeppelin-docs openzeppelin-docs/build-local.js' \
