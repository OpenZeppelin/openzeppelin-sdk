...
module.exports = function(dirname) {
  ...
  function getProxyAddress(network, name, index) {
    ...
    const fileName = path.resolve(
      dirname,
      `${workdirPath}/${getNetworkFileName(currentNetwork)}`
    );
    ...
    if (!proxyAddress) {
      throw new Error(
        `Address not found in proxy ${name}/${index}`,
        data.proxies[name][index]
      );
    }
  ...
  function execInWorkdir(cmd) {
    const output = spawnSync(cmd, {
      cwd: path.resolve(dirname, workdirPath),
      shell: true
    });
    if (output.status != 0 || output.error) {
      logOutput(output.stdout, output.stderr);
      throw new Error(
        `Error running ${cmd} (err ${output.status})\n${output.error}\n${String(
          output.stderr || ""
        )}`
      );
    }
  ...
  function copy(src, target) {
    fs.copyFileSync(
      path.resolve(dirname, `../files/${src}`),
      path.resolve(dirname, `${workdirPath}/${target}`)
    );
  }
