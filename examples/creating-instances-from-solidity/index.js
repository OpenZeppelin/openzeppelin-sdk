...
async function main() {
  ...
  if (fs.existsSync(".openzeppelin/project.json"))
    fs.unlinkSync(".openzeppelin/project.json");
  ...
  await init({
    name: "creating-instances-from-solidity",
    version: "0.1.0",
    publish: true
  });
  ...
  const factoryContract = Contracts.getFromLocal("Factory").at(
  ...
  const transactionReceipt = await factoryContract.methods
    .createInstance(data)
    .send(txParams);
  const uint256ToAddress = require("@openzeppelin/upgrades/lib/utils/Addresses.js")
    .uint256ToAddress;
  const instanceAddress = uint256ToAddress(
    transactionReceipt.events["0"].raw.data
  );
  ...
  const instanceContract = Contracts.getFromLocal("Instance").at(
    instanceAddress
  );
  ...
  const anotherAccount = (await ZWeb3.eth.getAccounts())[1];
  const value = await instanceContract.methods
    .value()
    .call({ ...txParams, from: anotherAccount });
